...
def test_salesforce_init_dev_env(mocker):
    ...
    mock_sf_instance = mocker.MagicMock(spec=SF)
    mocker.patch("viadot.sources.salesforce.SF", return_value=mock_sf_instance)
...
def test_salesforce_api_connection(mocker):
    ...
    mock_sf_instance.query.return_value = {"records": variables["records_v0"]}
...
def test_salesforce_api_connection_with_columns(mocker):
    ...
    mock_sf_instance.query.return_value = {"records": variables["records_v1"]}
...
def test_salesforce_to_df(mocker):
    ...
    mocker.patch("viadot.sources.salesforce.SF", return_value=mock_sf_instance)
    salesforce_instance = SalesForce(credentials=variables["credentials"])
    salesforce_instance.data = variables["data_v0"]
